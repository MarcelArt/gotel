...
func SetupAssetTransactionRoutes(v1 fiber.Router) {
	h := handlers.NewAssetTransactionHandler(
		services.NewAssetTransactionService(
			repositories.NewAssetTransactionRepo(configs.DB),
		),
	)
	h.SetupRoutes(v1)
}
