...
func NewUserRoleHandler(service IUserRoleService) *UserRoleHandler {
...
// @Accept       json
// @Produce      json
// @Param        user_role  body      UserRoleInput  true  "User role assignment details"
...
// @Success      200    {object}  paginate.Page{items=[]UserRolePage}
...
// @Param        user_role  body      UserRoleInput  true  "Updated user role mapping details"
...
func (h *UserRoleHandler) SetupRoutes(v1 fiber.Router) {
	...
	userRoles.Post("/", h.Create)
	userRoles.Get("/", h.Read)
	userRoles.Get("/:id", h.GetByID)
	userRoles.Put("/:id", h.Update)
	userRoles.Delete("/:id", h.Delete)
}
